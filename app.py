...
print(tf.__version__0)
# Read csv file containing data. 
df = pd.read_csv(r'D:\Users\Rejoy\Documents\GitHub\Emotion-based-music-recommendation-system\muse_v3.csv') 
print(df.head())
...
# Emotional = motional word used in that song. 
df = df.sort_values(by = ["emotional", "pleasant"]) 
df.reset_index()

# Giving dataframe based on emotional p pleasant value in sorted order. 
df_sad = df[:18000] 
...
# Task of function 'fun' is to take list of unique emotions 6 return dataframe of 30 rows. 
def fun(list):  
...
model.load_weights(r'D:\Users\Rejoy\Documents\GitHub\Emotion-based-music-recommendation-system\model.h5')
...
st.markdown("<h5 style = 'text-align: center; color: gray;'> <b>Click on the name of recommended song to reach website</b> </h5>", unsafe_allow_html = True)
    ...
    if st.button('SCAN EMOTION(Click here)'):
        ...
        while True:
            ...
            for (x, y, w, h) in faces:
                ...
                # Putting text of emotion on top of recangle
                cv2.putText(frame, emotion_dict[max_index], (x + 20, y - 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
                cv2.imshow('Video', cv2.resize(frame, (1800, 700), interpolation = cv2.INTER_CUBIC))
